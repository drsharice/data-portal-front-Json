...
// Use Vite's BASE_URL so this works on GitHub Pages subpaths AND Azure SWA root.
...
export default function Data() {
  ...
  // Load dataset list from static mock JSON
  useEffect(() => {
  ...
  // Load selected dataset rows from static mock JSON
  useEffect(() => {
    if (!selected) {
      setPreview(null);
      return;
    ...
    // File is named exactly after the key, e.g. Bloomberg_User_ReportTable.json
  ...
  // Client-side CSV download (from what's loaded in preview)
